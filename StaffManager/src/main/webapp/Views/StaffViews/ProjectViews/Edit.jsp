...
%>
<title><%=project.getName()%> | Sửa dự án |</title>
</head>
<body>
	...
	<div class="modal-dialog modal-dialog-centered modal-xl">
		<div class="modal-content">
			...
			<form action="/projects?type=edit&id=<%=project.getId()%>"
				method="post" id="prj-form-edit" onsubmit="lazySubmit(event)">
							...
							<div class="card mb-3">
								<div class="card-body">
									<div class="row">
										...
										<div class="col-sm-9 text-secondary">
											...
											<input type="hidden" class="form-control"
												value="<%=project.getName()%>" id="projectname"
												name="name">
										</div>
									</div>
									<hr>
									...
									<div class="row">
										...
										<div class="col-sm-9 text-secondary">
											<input type="date"
												value="<%=project.convertTime(project.getBeginDate(), 3)%>"
												id="begin-Date" onchange="getBeginDate()"> <input
												type="time"
												value="<%=project.convertTime(project.getBeginDate(), 4)%>"
												id="begin-Time" onchange="getBeginDate()"> <input
												type="hidden" id="beginDate"
												value="<%=project.convertTime(project.getBeginDate(), 3)%> <%=project.convertTime(project.getBeginDate(), 4)%>:00.000000000"
												name="beginDate">
										</div>
									</div>
									<hr>
...
=======
											<input type="date"
												value="<%=project.convertTime(project.getExpectedDate(), 3)%>"
												id="ex-Date" onchange="getExDate()"> <input
												type="time"
												value="<%=project.convertTime(project.getExpectedDate(), 4)%>"
												id="ex-Time" onchange="getExDate()"> <input
												type="hidden" id="expectedDate"
												value="<%=project.convertTime(project.getExpectedDate(), 3)%> <%=project.convertTime(project.getExpectedDate(), 4)%>:00.000000000"
												name="expectedDate">
>>>>>>> test_code_fix_balong
										</div>
									</div>
									<hr>
									<div class="row">
										...
										<div class="col-sm-9 text-secondary">
											<input type="date"
												value="<%=project.convertTime(project.getExpectedDate(), 3)%>"
												id="finish-Date" onchange="getFinishDate()"> <input
												type="time"
												value="<%=project.convertTime(project.getExpectedDate(), 4)%>"
												id="finish-Time" onchange="getFinishDate()"> <input
												type="hidden" id="finishDate"
												value="<%=project.convertTime(project.getExpectedDate(), 3)%> <%=project.convertTime(project.getExpectedDate(), 4)%>:00.000000000"
												name="finishDate">
										</div>
									</div>
						...
						<div class="col-md-12">
							...
							<div class="table-responsive m-3">
								<div class="prj-hst-change">
									<label
										class="prj-lb-hst text-uppercase font-weight-bold text-success"><%=history.getTitle()%></label>
									<hr>
									<p>
										<b>Ngày cập nhật : </b>
										<fmt:formatDate value="<%=history.getCreateAt()%>"
									...
									<div class="prj-hst-content">
										<label class="prj-lb-content font-weight-bold">Nội
											dung : </label>
					...
					<div class="row">
						<div class="col-12 py-3">
							<div class="card-header py-3">
								<h6 class="m-0 align-middle font-weight-bold text-primary">Chi
									chú</h6>
							</div>
						</div>
						<div class="col-12">
							<div class="row">
								...
								<div class="col-sm-9">
									<input type="text" class="form-control custom-select-sm"
										name="editTitle" placeholder="Tiêu đề chỉnh sửa"
										id="editTitle">
								</div>
							</div>
							<hr>
						...
						<div class="col-12">
							<div class="row">
								...
								<div class="col-sm-6">
									<input type="hidden" name="editAssignFrom"
										value="<%=project.getAccountId()%>"></input> <select
										name="editAssignTo"
										class="custom-select custom-select-sm form-control form-control-sm">
										<option value='<%=project.getAccountId()%>'>Choose
											..</option>
										<%
										...
										%>
										<option value="<%=i.getAccountId()%>"><%=i.getAccountName()%>
											-
											<%=i.getPositionName()%></option>
										<%
										}
										}
										%>
					...
					<div class="modal-footer">
						<a href="/projects?type=view&id=<%=project.getRoot()%>"
							type="button" class="btn btn-secondary">Hủy</a>
